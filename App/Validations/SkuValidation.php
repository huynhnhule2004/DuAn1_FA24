...
class SkuValidation
{
    ...
    public static function uploadImage1($files, $index)
    {
        ...
        $fileNameWithoutExtension = pathinfo($originalName, PATHINFO_FILENAME); // Lấy phần tên không bao gồm đuôi
        $cleanedFileName = preg_replace('/[^a-zA-Z0-9_-]/', '_', $fileNameWithoutExtension); // Thay ký tự đặc biệt bằng '_'
    
        // Đảm bảo phần mở rộng được giữ nguyên trong tên tệp
        $cleanedFileName .= '.' . $imageFileType; // Thêm phần mở rộng vào tên file đã làm sạch
        ...
        while (file_exists($target_file)) {
            // Thêm số đếm vào tên file nếu file đã tồn tại
